...
A saída do seu código também pode ser incorporada no texto, usando o modo **inline**. Este é o código usado apenas em uma linha, começando com
` `r ` e terminando com ` ` `. Usar o código inline é como este documento consegue saber que o conjunto de dados `cats` contém linhas `r nrow (cats)` (` contém ` r nrow(gatos) ` linhas `), e que o peso médio dos corações das fêmeas dos gatos é `r median(cats$Hwt[cats$Sex=="F"])` gramas (` `r median(cats$Hwt[cats$Sex=="F"])` `).
